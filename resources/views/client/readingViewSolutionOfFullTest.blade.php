...
@section('content')
    @if(Route::current()->getName() == 'readingViewResultLesson')
        @include('utils.readingViewResultTables', ['lesson' => $lesson_detail])
    @else
        @include('utils.readingViewSolutionTable', ['lesson' => $lesson_detail])
    @endif
    <div class="container solution-detail-page page-custom hidden" data-type-lesson-id="{!! $type_lesson_id !!}" >
        ...
        @foreach($paragraphs as $key_paragraph => $paragraph)
            <div class="solution-detail paragraph-detail panel-container {!! ($paragraph->order_paragraph != 1) ? 'hidden' : '' !!} transform-custom paragraph-detail-{!! $paragraph->order_paragraph !!}" data-order-paragraph="{!! $paragraph->order_paragraph !!}" data-paragraph-id="{!! $paragraph->id !!}">
                <div class="left-panel-custom panel-left-{!! $paragraph->order_paragraph !!} panel-top-{!! $paragraph->order_paragraph !!}" id="lesson-highlight-area" data-lessonid="{!! $lesson_detail->id !!}">
                    {!! $paragraph->content_highlight !!}
                </div>
                <div class="splitter-custom splitter-{!! $paragraph->order_paragraph !!}">
                </div>
                <div class="splitter-horizontal-custom splitter-horizontal-{!! $paragraph->order_paragraph !!}">
                </div>
                <div class="right-panel-custom panel-right-{!! $paragraph->order_paragraph !!} panel-bottom-{!! $paragraph->order_paragraph !!} active-quiz" id="solution-area" data-quizId="{!! $lesson_detail->id !!}">
                    {!! $paragraph->content_answer_quiz !!}
                </div>
            </div>
        @endforeach
...
@section('scripts')
    ...
    @if(Route::current()->getName() == 'readingViewSolutionLesson')
