...
<table id="reading-list-lesson" class="table datatable display manager-lesson-table">
    ...
    @foreach($lessons as $key => $lesson)
        <?php
        $type_lesson_id = config('constants.type_lesson.mini_test');
        ...
        $all_orders = $readingLessonService->getAllOrderLesson($type_lesson_id, $lesson->typeQuestion->id);
        $created_at = timeFormat($lesson->created_at);
        $type_lesson = 'MiniTest';
        ?>
        <tr class="item_row item-lesson-{!! $lesson->id !!}">
            ...
            <td>
                @include('components.modals.editTypeQuestionModal', ['all_level_lessons' => $all_level_lessons, 'all_type_questions' => $all_type_questions, 'lesson' => $lesson, 'type_lesson_id' => $type_lesson_id, 'all_orders' => $all_orders, 'type_lesson' => $type_lesson])
            </td>
            <td>
                @include('components.modals.editLevelLessonModal', ['lesson' => $lesson, 'type_lesson' => $type_lesson])
            </td>
            <td>
