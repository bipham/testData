        ...
        <div class="header-left-menu">
            <div class="center-class logo-website-left-menu">
                <a href="{{url('/')}}" class="brand-web">
                    <img src="{{url('/imgs/original/logo.jpg')}}" alt="Logo" class="img-custom img-logo-website">
                </a>
            </div>
        </div>
        <div class="body-left-menu">
            ...
            <ul id="menu-content" class="menu-content">
                ...
                @if(sizeof($all_introductions) > 0)
                    ...
                    <h6 class="title-menu" data-toggle="collapse" data-target="#introduction" aria-expanded="@if($type_question_id_current < 0) true @else false @endif">
                            ...
                            <li class="item-lesson level-one @if($introduction->id == $lesson_id_current) current-step @endif">
                                <span class="pull-left title-lesson-menu">
                                    <a href="{{url('/reading/' . $current_level_lesson->id . '-level/readingViewLearning/' . config('constants.type_lesson.learning') . '-introduction/' . $introduction->id . '-introduction')}}">
                                        {!! $key_introduction + 1 !!}. {!! $introduction->title !!}
                                    </a>
                                </span>
                                <span class="pull-right tools-area-menu">
                                    <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.learning')  . 'introduction-' . $introduction->id)}}" class="badge badge-success link-solution-lesson">
                                        <i class="fa fa-key" aria-hidden="true"></i> Solution
                                    </a>
                                    @if($highest_score == null)
                                        <i class="fa fa-unlock" aria-hidden="true"></i>
                                    @else
                                        <span class="badge badge-primary">{!! $highest_score->highest_correct !!}/{!! $introduction->total_questions !!}</span>
                                    @endif
                                </span>
                            </li>
                        @endforeach
                ...
                @if(sizeof($all_lessons) > 0)
                    <div class="lesson-area show-lesson-area {!! $level_lesson_id == 3 ? 'hidden' : '' !!}">
                        <h6 class="title-menu" data-toggle="collapse" data-target="#lessons" aria-expanded="false">
                            <a href="#">Lessons</a>
                        </h6>
                        <ul class="primary-menu collapse @if($type_lesson_id >= 0 && $type_lesson_id < config('constants.type_lesson.mix_test') && $type_question_id_current >= 0) show type-current @endif" id="lessons">
                            ...
                            @if(sizeof($all_learnings) > 0 || sizeof($all_practices) > 0 || sizeof($all_mini_tests) > 0)
                                ...
                                <ul class="sub-menu sub-level-one collapse @if($lesson->id == $type_question_id_current) show @endif" id="lesson-{!! $lesson->id !!}">
                                    ...
                                    @if(sizeof($all_learnings) > 0)
                                        <?php
                                        $learning_highest_step = $readingStatusLearningOfUserService->getHighestStepLessonService($current_level_lesson->id, $lesson->id, config('constants.type_lesson.learning'));
                                        ?>
                                        ...
                                        @foreach($all_learnings as $learning)
                                                ...
                                                ?>
                                                <li class="item-lesson @if($learning->id == $lesson_id_current) current-step @endif @if($learning->order_lesson > $learning_highest_step) disabled-lesson @endif">
                                                    @if($check_vip)
                                                        <span class="pull-left title-lesson-menu">
                                            {!! $learning->title !!}
                                        </span>
                                                        <span class="pull-right tools-area-menu">
                                            <a href="#" class="link-upgrade-user">
                                                <span class="badge badge-danger pro-badge">PRO</span>
                                            </a>
                                        </span>
                                            ...
                                            <a href="{{url('/reading/' . $level_lesson_id . '-level/readingViewLearning/' . config('constants.type_lesson.learning') . '-learning/' . $learning->id . '-learning')}}">
                                                {!! $learning->title !!}
                                                        ...
                                                        <span class="pull-right tools-area-menu">
                                                        <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.learning')  . 'learning-' . $learning->id)}}" class="badge badge-success link-solution-lesson">
                                                            <i class="fa fa-key" aria-hidden="true"></i> Solution
                                                        </a>
                                                            @if($highest_score == null)
                                                                <i class="fa fa-unlock" aria-hidden="true"></i>
                                                            @else
                                                                <span class="badge badge-primary">{!! $highest_score->highest_correct !!}/{!! $learning->total_questions !!}</span>
                                                            @endif
                                                    </span>
                                                    @endif
                                                </li>
                                        ...
                                        @foreach($all_practices as $practice)
                                            ...
                                            <a href="{{url('/reading/' . $level_lesson_id . '-level/readingLesson/' . config('constants.type_lesson.practice') . '-practice/' . $practice->id . '-practice')}}">
                                                {!! $practice->title !!}
                                                    ...
                                                    <span class="pull-right tools-area-menu">
                                                        <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.practice')  . 'practice-' . $practice->id)}}" class="badge badge-success link-solution-lesson">
                                                            <i class="fa fa-key" aria-hidden="true"></i> Solution
                                                        </a>
                                                        @if($highest_score == null)
                                                            <i class="fa fa-unlock" aria-hidden="true"></i>
                                    ...
                                    @if(sizeof($all_mini_tests) > 0)
                                        <?php
                                        $mini_test_highest_step = $readingStatusLearningOfUserService->getHighestStepLessonService($current_level_lesson->id, $lesson->id, config('constants.type_lesson.mini_test'));
                                        ?>
                                        ...
                                        @foreach($all_mini_tests as $mini_test)
                                            ...
                                            ?>
                                            <li class="item-lesson @if($mini_test->id == $lesson_id_current) current-step @endif @if($mini_test->order_lesson > $mini_test_highest_step) disabled-lesson @endif">
                                                @if($check_vip)
                                                    <span class="pull-left title-lesson-menu">
                                                    {!! $mini_test->title !!}
                                                </span>
                                                ...
                                                @else
                                                    ...
                                                    <span class="pull-right tools-area-menu">
                                                    <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.mini_test')  . 'miniTest-' . $mini_test->id)}}" class="badge badge-success link-solution-lesson">
                                                        <i class="fa fa-key" aria-hidden="true"></i> Solution
                                                    </a>
                                                        @if($highest_score == null)
                                                            <i class="fa fa-unlock" aria-hidden="true"></i>
                        ...
                        @foreach($mix_tests as $key_mix_test => $mix_test)
                            ...
                            <li class="item-lesson level-one @if($mix_test->id == $lesson_id_current) current-step @endif @if($mix_test->order_lesson > 1) disabled-lesson @endif">
                                ...
                                @else
                                    ...
                                    <span class="pull-right tools-area-menu">
                                        <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.mix_test')  . 'mix_test-' . $mix_test->id)}}" class="badge badge-success link-solution-lesson">
                                            <i class="fa fa-key" aria-hidden="true"></i> Solution
                                        </a>
                                        @if($highest_score == null)
                                            <i class="fa fa-unlock" aria-hidden="true"></i>
                ...
                @if(sizeof($full_tests) > 0)
                    ...
                    <ul class="primary-menu collapse @if($type_lesson_id == config('constants.type_lesson.full_test')) show type-current @endif" id="fullTest">
                        ...
                        @foreach($full_tests as $key_full_test => $full_test)
                            <?php
                            $check_vip = $readingLessonService->checkVipLesson(config('constants.type_lesson.full_test'), $full_test->id);
                            $highest_score = $readingResultService->getHighestScoreLesson(config('constants.type_lesson.full_test'), $full_test->id);
                            ?>
                            <li class="item-lesson level-one @if($full_test->id == $lesson_id_current) current-step @endif @if($full_test->order_lesson > 1) disabled-lesson @endif">
                                ...
                                @else
                                    ...
                                    <span class="pull-right tools-area-menu">
                                        <a href="{{url('/reading/'. $current_level_lesson->id . '-level/readingViewSolutionLesson/' . config('constants.type_lesson.full_test')  . 'full_test-' . $full_test->id)}}" class="badge badge-success link-solution-lesson">
                                            <i class="fa fa-key" aria-hidden="true"></i> Solution
                                        </a>
                                        @if($highest_score == null)
                                            <i class="fa fa-unlock" aria-hidden="true"></i>
