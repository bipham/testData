...
class ReadingResultService
{
    ...
    public function getResultLesson($type_lesson_id, $lesson_id, $list_answered)
    {
        ...
        if ($list_answered != 'emptyList') {
            foreach ($list_answered as $question_custom_id => $answer_key) {
                ...
                if ($type_lesson_id == 0) {
                    $answer_extractly = $this->_readingQuestionLearningModel->getAnswerExtractlyOfQuestion($question_custom_id);
                }
                else {
    ...
    public function getTopHighestScores($type_lesson_id, $lesson_id_current) {
        return $this->_readingResultLessonModel->getTopUserScore($type_lesson_id, $lesson_id_current);
    }
}
?>