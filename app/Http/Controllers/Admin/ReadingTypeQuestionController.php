...
class ReadingTypeQuestionController extends Controller
{
    ...
    public function getTypeQuestionForEditTest($domain) {
        $level_lesson_id = $_GET['level_lesson_id'];
        $lesson_id = $_GET['lesson_id'];
        $type_lesson_id = $_GET['type_lesson_id'];
        $readingTypeQuestionService = new ReadingTypeQuestionService();
        $readingQuestionLessonService = new ReadingQuestionLessonService();
        $all_type_questions = $readingTypeQuestionService->getAllTypeQuestionById($level_lesson_id);
        $list_type_question_details = $readingQuestionLessonService->getListTypeQuestionDetail($type_lesson_id, $lesson_id);
        return json_encode(['list_type_questions' => $all_type_questions, 'list_type_question_details' => $list_type_question_details]);
    }
