...
class ReadingResultLesson extends Model
{
    ...
    public function getAllResultOfUser($user_id) {
        return $this->where('user_id', $user_id)->select('lesson_id', 'type_lesson_id', 'highest_correct')->orderBy('created_at', 'desc')->get()->all();
    }

    public function getAllResultOverDetailOfUser($user_id, $type_lesson_id) {
        return $this->where('user_id', $user_id)->where('type_lesson_id', $type_lesson_id)->select('lesson_id', 'type_lesson_id', 'highest_correct')->orderBy('created_at', 'desc')->get()->all();
    }

    public function getTopUserScore($type_lesson_id, $lesson_id_current) {
        return $this->where('type_lesson_id', $type_lesson_id)->where('lesson_id', $lesson_id_current)->select('user_id', 'highest_correct')->orderBy('highest_correct', 'desc')->take(5)->get()->all();
    }
}
