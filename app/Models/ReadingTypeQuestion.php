...
class ReadingTypeQuestion extends Model
{
    ...
    public function getTypeQuestionName($type_question_id) {
        return $this->where('status', 1)->where('id', $type_question_id)->select('name')->get()->first();
    }

    public function getLevelLessonIdByTypeQuestion($type_question_id) {
        return $this->where('status', 1)->where('id', $type_question_id)->select('level_lesson_id')->get()->first();
    }
}
