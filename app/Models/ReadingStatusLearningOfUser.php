...
class ReadingStatusLearningOfUser extends Model
{
    ...
    public function getLessonFinishedByLevelLesson($level_lesson_id, $user_id) {
        return $this->where('level_lesson_id', $level_lesson_id)->where('user_id', $user_id)->get()->sum(function ($product) {
            return ($product->step_current - 1);
        });
    }
}
