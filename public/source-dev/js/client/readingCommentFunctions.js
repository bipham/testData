...
var type_lesson_id = $('.solution-detail-page').data('type-lesson-id');

function showExplanation(question_custom_id, question_order, isTrigger) {
    "use strict";
    isTrigger = isTrigger || false;
    var ajaxShowExplanation = baseUrl + '/showExplanation/' + type_lesson_id + '/' + question_custom_id;
    $.ajax({
        type: "GET",
        ...
        success: function (data) {
            ...
            if ($('.highlight-' + question_order).length) {
...
function enterComment(e) {
    if (e.keyCode == 13) {
        ...
        if (content_cmt != '') {
            ...
            var ajaxUrlEnterReplyComment = baseUrl + '/enterNewComment/' + type_lesson_id;
    ...
    if (current_level_user == 1) {
        if (reply_comment_id == 0) {
            $('#commentArea .comments-area').append('<div class="row list-cmt-area list-cmt-' + comment_id + '" data-cmt-id="' + comment_id + '">' +
                ...
                + '<img alt="time-cmt" src="/imgs/original/time.png" class="img-time-cmt" />'
                + '</span>'
